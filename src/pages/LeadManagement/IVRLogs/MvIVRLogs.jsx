...
// import { leadsColumn } from '../../../components/TableHeader';
// import { useNavigate } from 'react-router-dom';
// import { getLeads, getMviIVRLogs } from '../../../api-services/Modules/Leads';
// import * as XLSX from 'xlsx';
// import { saveAs } from 'file-saver';
...
const Leads = () => {
  const navigate = useNavigate();
  const [rawData, setRawData] = useState([]); // Original API Data
  const [loading, setLoading] = useState(false);
  // ✨ NEW STATE to hold the fully filtered, non-paginated data for export
  ...
  const { tableData, filteredCount } = useMemo(() => {
    ...
    // ✨ UPDATE THE EXPORT DATA STATE WITH THE FULLY FILTERED LIST
    ...
    return { tableData: pageData, filteredCount: count };
  }, [rawData, query]); // Added setExportDataList to dependency array

  // ... (Other handlers like onPageChange, onSearchHandler, debouncedSearch, onFilterByDate, onFilterByRange, handleStatusFilter remain unchanged) ...
  ...
  return (
    <>
      ...
      <DataTable
        ...
        onSearch={debouncedSearch}
        onRefresh={fetchLeads}
        onExport={handleExport} // Using the modified export handler
        onCreate={() => navigate('/leads/create')}
        createLabel="Add Lead"
        title="IVR MV LOGS"
