...
import { getKBMumbaiLogs } from '../../api-services/Modules/Leads';
...
const KBMumbai = () => {
  ...
  const fetchLeads = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getKBMumbaiLogs({
  ...
  const handleExportSubmit = async ({ startDate, endDate, mode }) => {
    ...
    try {
      ...
      const url = `${import.meta.env.VITE_API_URL}/leads/kb-success-leads-export-mumbai?${urlParams.toString()}`;
  ...
  const handleEdit = (lead) => {
    ...
    navigate(`/kb-mumbai-success-leads/${lead.id}`, { state: { lead } });
  };
